//
//  JLPTN5SelectViewController.swift
//  JLPT ALL TEST
//
...
class JLPTN5SelectViewController: UIViewController {
    ...
    private func setupUI() {
        setupButton(button: readingButton, action: #selector(openQuizView), iconName: "book.fill", color: .red)
        setupButton(button: listeningButton, action: #selector(openListeningQuizView), iconName: "headphones", color: .red)

        setupLabel(label: readingLabel, text: "JLPT N5")
        setupLabel(label: listeningLabel, text: "JLPT N5")
    ...
    @objc private func openQuizView() {
        let quizVC = JLPTN5ViewController()
        present(quizVC, animated: true)
    }
    
    @objc private func openListeningQuizView() {
        let listeningQuizVC = JLPTN5ListeningViewController()
