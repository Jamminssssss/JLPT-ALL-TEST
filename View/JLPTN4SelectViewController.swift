//
//  JLPTN4SelectViewController.swift
//  JLPT ALL TEST
//
...
class JLPTN4SelectViewController: UIViewController {
    ...
    private func setupUI() {
        setupButton(button: readingButton, action: #selector(openQuizView), iconName: "book.fill", color: .green)
        setupButton(button: listeningButton, action: #selector(openListeningQuizView), iconName: "headphones", color: .green)

        setupLabel(label: readingLabel, text: "JLPT N4")
        setupLabel(label: listeningLabel, text: "JLPT N4")
    ...
    @objc private func openQuizView() {
        let quizVC = JLPTN4ViewController()
        present(quizVC, animated: true)
    }
    
    @objc private func openListeningQuizView() {
        let listeningQuizVC = JLPTN4ListeningViewController()
