//
//  JLPTN2SelectViewController.swift
//  JLPT ALL TEST
//
...
class JLPTN2SelectViewController: UIViewController {
    ...
    private func setupUI() {
        setupButton(button: readingButton, action: #selector(openQuizView), iconName: "book.fill", color: .blue)
        setupButton(button: listeningButton, action: #selector(openListeningQuizView), iconName: "headphones", color: .blue)

        setupLabel(label: readingLabel, text: "JLPT N2")
        setupLabel(label: listeningLabel, text: "JLPT N2")
    ...
    @objc private func openQuizView() {
        let quizVC = JLPTN2ViewController()
        present(quizVC, animated: true)
    }
    
    @objc private func openListeningQuizView() {
        let listeningQuizVC = JLPTN2ListeningViewController()
